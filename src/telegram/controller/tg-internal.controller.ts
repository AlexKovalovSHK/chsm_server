...
export class TgInternalController {
  ...
  async updateStep(@Body() dto: { tgId: string; step: string }) {
    return await this.tgInternalService.updateRegistrationStep(
      dto.tgId,
      dto.step,
    );
  }
  ...
  @Post('broadcast')
  async broadcast(@Body() dto: { users: string[]; text: string }) {
  ...
  async broadcastExtension(@Body() dto: { users: string[]; text: string }) {
    const results = {
      sent: 0,
      failed: 0,
