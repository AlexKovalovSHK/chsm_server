...
export class McpRegistryService implements OnApplicationBootstrap {
  ...
  private discoverTools() {
    const allModules = Array.from(this.modulesContainer.values());

    // Find any McpModule instance to get its ID
    const mcpModule = allModules.find(
      (m) => (m.instance as any)?.__isMcpModule,
    );

    if (!mcpModule) {
      this.logger.warn('No McpModule found in the application container.');
      return;
    }
    ...
    this.logger.debug(
      `Discovering all tools for MCP Module ID: ${mcpModuleId}`,
    );

    this.discoverToolsForModuleSubtree(mcpModuleId, allModules);
  }
