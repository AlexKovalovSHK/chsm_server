...
import { ChatController } from './transport/chat.controller';
...
@Global()
@Module({
  imports: [DiscoveryModule, ConfigModule.forRoot({ isGlobal: true })],
  providers: [
    {
      provide: 'MCP_MODULE_ID',
      useValue: 'mcp-default-module',
    },
    // MCP_OPTIONS placeholder for static context (will be overridden by forRoot)
    {
      provide: 'MCP_OPTIONS',
      useValue: { name: 'placeholder', version: '0.0.0' },
    },
    McpRegistryService,
    AiBridgeService,
  ],
...
export class McpModule {
  ...
  static forRoot(options: McpOptions): DynamicModule {
    ...
    // Dynamically provide all services that depend on MCP_OPTIONS
  ...
  static forRootAsync(options: McpModuleAsyncOptions): DynamicModule {
    ...
    // Note: asyncProviders already provides 'MCP_OPTIONS' via useFactory.
    // McpExecutorService and ToolAuthorizationService depend on it.
